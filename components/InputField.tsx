...
const InputField: React.FC<InputFieldProps> = ({ label, id, value, onChange, placeholder, unit }) => {
    return (
        <div className="mb-5 group">
            <label htmlFor={id} className="block text-sm font-semibold text-slate-700 mb-2 transition-colors group-focus-within:text-blue-600">
                {label}
            </label>
            <div className="relative">
                <input
                    ...
                    className="w-full px-4 py-3.5 bg-gradient-to-br from-white to-slate-50 border-2 border-slate-200 rounded-xl shadow-sm transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 focus:shadow-lg focus:shadow-blue-500/10 hover:border-slate-300 placeholder:text-slate-400"
                />
                {unit && (
